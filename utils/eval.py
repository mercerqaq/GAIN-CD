...
def set_seed(seed):
    ...
    torch.cuda.manual_seed_all(seed)  ##删除tf这一行则取消这行注释
    try:
        os.environ['PYTHONHASHSEED'] = str(seed)
    except:
        pass

def print_edges(B, k = 10):##打印图B的前K条边
    edges = np.where(B)
    N = edges[0].shape[0]
    k = min(k, N)
    for i in range(k):
...
def threshold_till_dag(B):##判断输入矩阵B是否是一个DAG，如果不是则递归删除最小权重的边直到得到一个DAG
...
def postprocess(B, graph_thres=0.3):#设置边阈值，将小于graph_thres的边设为0，再用threshold_till_dag()方法将矩阵修改为DAG
...
class MetricsDAG(object):##计算估计的DAG图与真实图之间的各种准确性度量
    """
...
def evaluate(B_true, B_est, threshold,return_edges = False, prune = True):##评估估计的图 B_est 与真实图 B_true 的相似度
...
def write_result(result_dict, config_code, saved_path):##将评估结果写入指定的文件
