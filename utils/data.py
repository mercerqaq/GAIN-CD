"""
The function produce_NA is from https://rmisstastic.netlify.app/how-to/python/generate_html/how%20to%20generate%20missing%20values
The other functions are from https://github.com/BorisMuzellec/MissingDataOT/blob/master/utils.py
"""
...
def produce_NA(X, p_miss, mecha="mcar", opt=None, p_obs=None, q=None):
    ...
    """
    d = X.shape[1]
    to_torch = torch.is_tensor(X)  # 检查数据是否是torch张量，不是则转化为torch张量
    if not to_torch:
        X = X.astype(np.float32)
        X = torch.from_numpy(X)
    #根据缺失机制选择缺失生成方式
    if mecha == "mar":#随机缺失
        mask = MAR_mask(X, p_miss, p_obs)
    elif mecha == "mnar" and opt == "logistic":#使用逻辑回归、分位数裁剪、自遮掩机制生成缺失
        mask = MNAR_mask_logistic(X, p_miss, p_obs)
    elif mecha == "mnar" and opt == "quantile":
        mask = MNAR_mask_quantiles(X, p_miss, q, 1 - p_obs)
    elif mecha == "mnar" and opt == "selfmasked":
        mask = MNAR_self_mask_logistic(X, p_miss)
    elif mecha == "mnar_with_p_obs" and opt == "selfmasked":
        ...
        mask = MNAR_self_mask_logistic(X, p_miss)
        mask[:, idxs_obs] = False
    elif mecha == "mcar_with_p_obs":#完全随机缺失中确保部分变量可观测
        # number of variables that will have no missing values (at least one variable)
        d_obs = max(int(p_obs * d), 1)
        # Sample variables that will all be observed, and those with missing values:
        idxs_obs = np.random.choice(d, d_obs, replace=False)
        mask = (torch.rand(X.shape) < p_miss)
        mask[:, idxs_obs] = False
    else:
        mask = (torch.rand(X.shape) < p_miss)
    ...
    return X_nas.numpy().astype(np.float32), (~mask).numpy().astype(bool)#得到缺失数据矩阵和掩码矩阵


def nanmean(v, *args, **kwargs):#计算包含NaN的pytorch张量均值
    """
...
#### Quantile ######
def quantile(X, q, dim=None):#计算pytorch张量的第q分位数
    """
...
def pick_epsilon(X, quant=0.5, mult=0.05, max_points=2000):#用于估计计算Sinkhorn距离时使用的正则化参数，通过对输入数据的成对距离计算进行分位数估计，来选择一个合适的正则化参数。
    """
    ...
    X_ = X.clone()
    mask = torch.isnan(X_)
    X_[mask] = (mask * means)[mask]#用每列数据的均值填补缺失值

    idx = np.random.choice(len(X_), min(max_points, len(X_)), replace=False)
    X = X_[idx]
    dists = ((X[:, None] - X) ** 2).sum(2).flatten() / 2.#计算X的成对平方欧几里得距离
    dists = dists[dists > 0]#移除对角线上的距离

    return quantile(dists, quant, 0).item() * mult


#### Accuracy Metrics ####
def MAE(X, X_true, mask):#计算插补后数据和真实数据间的平均绝对误差
    """
...
def RMSE(X, X_true, mask):#计算插补后数据和真实数据间的均方根误差
    """
...
def MAR_mask(X, p, p_obs):#实现 MAR 机制，使用逻辑回归模型生成缺失。首先随机选择一部分变量（特征）不缺失，其他变量的缺失则由逻辑回归模型决定，缺失比例是根据输入参数 p 控制的。
    """
    ...
    p : float 需要生成缺失值的比例（用于控制缺失比例）
        Proportion of missing values to generate for variables which will have missing values.
    p_obs : float  使用逻辑回归模型时不缺失的变量所占的比例
    ...
    mask = torch.zeros(n, d).bool() if to_torch else np.zeros(
        (n, d)).astype(bool)
    ...
    # 随机选择 d_obs 个变量作为不缺失的变量，将它们的索引存储在 idxs_obs 中。剩余的变量（将缺失的）索引存储在 idxs_nas 中。
    idxs_obs = np.random.choice(d, d_obs, replace=False)
    ...
    return mask#返回生成的缺失数据掩码矩阵 mask，其中 True 表示数据缺失。


##### Missing not at random ######

def MNAR_mask_logistic(X, p, p_params=.3, exclude_inputs=True):#使用逻辑回归实现MNAR的缺失机制
    """
    ...
    idxs_params = np.random.choice(
        d, d_params, replace=False) if exclude_inputs else np.arange(d)
    idxs_nas = np.array([i for i in range(
        d) if i not in idxs_params]) if exclude_inputs else np.arange(d)
...
def MNAR_self_mask_logistic(X, p):#通过自遮掩的逻辑回归模型实现MNAR
    """
...
def MNAR_mask_quantiles(X, p, q, p_params, cut='both', MCAR=False):#使用分位数裁剪实现MNAR
    """
    ...
    # number of variables that will have NMAR values
...
def create_mcar_mask(n, d, miss_percent=0.1):
    total_entries = n * d
    mask = np.full(total_entries, True)
    mask[:int(total_entries * miss_percent)] = False
    np.random.shuffle(mask)
    mask = mask.reshape(n, d)
    return mask


def create_mar_mask(X, miss_percent=0.1):
    raise NotImplementedError(
        "Function create_mar_mask has not been implemented.")


def create_mnar_mask(X, miss_percent=0.1):
    raise NotImplementedError(
        "Function create_mnar_mask has not been implemented.")
