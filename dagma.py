...
class DagmaMLP(nn.Module): 
    ...
    def __init__(self, dims, device, bias = True, dtype: torch.dtype = torch.float32):
        r"""
        Parameters
        ----------
        ...
        self.I = torch.eye(self.d, device=device).to(dtype)
        self.fc1 = nn.Linear(self.d, self.d * dims[1], bias=bias).to(dtype)#第一个全连接层
        nn.init.zeros_(self.fc1.weight)#初始权重和偏置都置为0
        nn.init.zeros_(self.fc1.bias)
        # fc2: local linear layers局部连接层
        layers = []
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:  
        ...
        #x = torch.from_numpy(x).to(self.fc1.weight.device).to(self.fc1.weight.dtype)##用GAIN填补的时候取消注释
        x = self.fc1(x)##输入数据传入第一层全连接层fc1
        x = x.view(-1, self.dims[0], self.dims[1])#重塑为三维张量，以便输入到局部连接层
        for fc in self.fc2:##x通过每一层fc2并应用sigmoid激活
            x = torch.sigmoid(x)
            x = fc(x)
        x = x.squeeze(dim=2)
        return x

    def h_func(self, s: float = 1.0) -> torch.Tensor:##计算无环约束
        r"""
    ...
    def fc1_l1_reg(self) -> torch.Tensor:##计算第一层全连接层的L1正则化，防止过拟合
        r"""
    ...
    @torch.no_grad()
    def fc1_to_adj(self) -> np.ndarray:  # [j * m1, i] -> [i, j]##将权重矩阵转换为加权邻接矩阵
        r"""
