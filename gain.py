...
from utilsgain import TrainHistory, EarlyStopper
...
class GAIN:
    ...
    def __init__(self,
        ...
        #x_data, m_data = iter(train_loader).next()  用DAGMA-gaintroch环境就用这行代码
        #x_data, m_data = next(iter(train_loader))#用OTM cpu版本就用这行代码
        ...
        if train_loader is not None:
            ...
            if x_data.shape[1] != m_data.shape[1]:
                raise ValueError('@x_data and @m_data must have the same number of features')
            n_features_final = x_data.shape[1]
        elif n_features is not None:
            n_features_final = n_features
        else:
            raise ValueError("Either train_loader or n_features must be provided")

        if seed is not None:
            self._set_seed(seed)

        self.G = Generator(n_features_final).to(self.device)
        self.D = Discriminator(n_features_final).to(self.device)
    ...
    def train(self, n_epoches: int, stopper: EarlyStopper = None,  verbose=True) -> None:
        ...
        for epoch in range(n_epoches):
            ...
            for X_batch, M_batch in t:
                ...
                X_new = M * X_old + (1 - M) * Z###生成缺失数据的填补矩阵
                ...
                # 2-й этап: оптимизация генератора при фиксированном дискриминаторе生成器训练
                G_loss, MSE_train, MSE_test = self._generator_step(x_old=X_old, x_new=X_new, m=M, h=H)
                '''
                G_weighted = torch.matmul(G_sample, W_est)  # 生成数据与邻接矩阵的矩阵乘法
                structure_loss = torch.mean((G_weighted - G_sample) ** 2)  
                G_loss += alpha * structure_loss
                '''
                G_loss.backward()
    ...
    def imputation(self, x: torch.Tensor, m: torch.Tensor) -> torch.Tensor:
        ...
        print('imputation_x：',x)
        m = m.to(self.device)
        print('imputation_m：', m)
        self.G.eval()
        with torch.no_grad():
            x_imputed = self.G(torch.cat(tensors=[x, m], dim=1))
            x_hat = m * x + (1 - m) * x_imputed
            print('imputation_xhat：', x_hat)
            return x_hat
    ...
    #def _generator_step(self, x_old: torch.Tensor, x_new: torch.Tensor, m: torch.Tensor, h: torch.Tensor,
                               #W_est: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    def _generator_step(self, x_old: torch.Tensor, x_new: torch.Tensor, m: torch.Tensor, h: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Updates the generator parametersde
        ...
        loss = -torch.mean((1 - m) * torch.log(m_hat + 1e-8)) + self.alpha * mse_train##gain的原本loss
        ##新的生成器loss
        '''
        structure_loss = torch.mean((torch.matmul(x_hat, W_est) - x_hat) ** 2)  # 生成数据与邻接矩阵之间的差异
        loss = -torch.mean((1 - m) * torch.log(m_hat + 1e-8)) + self.alpha * mse_train + structure_loss#self.alpha * structure_loss
        '''
        m_inv = 1 - m
    ...
    def forward_impute(self, x: torch.Tensor, m: torch.Tensor, z: torch.Tensor = None):
        """
        x: 原始数据，NaN 位置可先置 0
        m: mask，当前 gain.py 的语义是 1=观测, 0=缺失
        z: 缺失位置随机噪声
        """
        x = torch.nan_to_num(x, nan=0).to(self.device)
        m = m.to(self.device)
        ...
        x_new = m * x + (1 - m) * z
        x_imputed = self.G(torch.cat([x_new, m], dim=1))
        x_hat = m * x + (1 - m) * x_imputed
        return x_hat, x_imputed, x_new

    def discriminator_loss(self, x: torch.Tensor, m: torch.Tensor, h: torch.Tensor, z: torch.Tensor = None):
        x_hat, _, _ = self.forward_impute(x, m, z)
        h = h.to(self.device)
        m_hat = self.D(torch.cat([x_hat, h], dim=1))
        loss = -torch.mean(m * torch.log(m_hat + 1e-8) + (1 - m) * torch.log(1 - m_hat + 1e-8))
        return loss

    def generator_loss(self, x: torch.Tensor, m: torch.Tensor, h: torch.Tensor, z: torch.Tensor = None):
        x = torch.nan_to_num(x, nan=0).to(self.device)
        m = m.to(self.device)
        h = h.to(self.device)

        x_hat, x_imputed, x_new = self.forward_impute(x, m, z)
        m_hat = self.D(torch.cat([x_hat, h], dim=1))

        absolute_error_train = m * x_new - m * x_imputed
        mse_train = torch.mean(absolute_error_train * absolute_error_train) / (torch.mean(m) + 1e-8)

        adv_loss = -torch.mean((1 - m) * torch.log(m_hat + 1e-8))
        loss = adv_loss + self.alpha * mse_train

        m_inv = 1 - m
        absolute_error_test = m_inv * x - m_inv * x_imputed
        mse_test = torch.mean(absolute_error_test * absolute_error_test) / (torch.mean(m_inv) + 1e-8)

        return loss, mse_train, mse_test, x_hat
